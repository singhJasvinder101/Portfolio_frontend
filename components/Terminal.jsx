...
import { SITE_ROLE, SITE_COMPANY, SITE_BUILDING } from "@/lib/site-config";
...
export default function HeroTerminal() {
    ...
    const [doneLines, setDoneLines] = useState(() => (reduceMotion ? heroLines : []));
    const [typingIdx, setTypingIdx] = useState(() => (reduceMotion ? heroLines.length : 0));
    ...
    const currentLine = heroLines[typingIdx];
...
function getHeroLines() {
    return [
        ...
        { plain: `  Role:     "${SITE_ROLE} @ ${SITE_COMPANY}",`, key: true, field: "Role" },
        { plain: '  Stack:    []string{"Go", "Python", "Kafka", "LangGraph"},', key: true, field: "Stack" },
        { plain: `  Building: "${SITE_BUILDING}",`, key: true, field: "Building" },
        { plain: "}", key: true },
        { plain: "" }
    ];
}
